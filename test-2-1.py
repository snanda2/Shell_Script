...
def print_response_fields(response_parts):
    ...
    response_codes = [str(part.get('responseCode')) for part in response_parts if part]
    ...
    for response_code, count in total_counts.items():
        try:
            ...
            response_part = next(part for part in response_parts if part.get('responseCode') == int(response_code))

            # Extract desired fields
            response_message = str(response_part.get('responseMessage', ''))
            host_response_code = str(response_part.get('hostResponseCode', ''))
